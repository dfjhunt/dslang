 ...
 * @param <B> - right type
 */
public class Either<A, B> implements Monad<Either.t<A>, B>, BiFunctor<A, B>, SplitFunctor<A, B> {
     ...
     */
    @Override
    public <U> Monad<Either.t<A>, U> unit(U u) {
        return right(u);
    }
     ...
     */
    @Override
    public <U> Either<A, U> flatMap(Function<? super B, ? extends Monad<Either.t<A>, U>> mapper) {
        if (_isLeft)
            return left(_left);
        else
